...
function MessageBubble({ sender, text, attachment, type }) {
  ...
  const alignmentClass = isStatus || isBot ? 'justify-start' : 'justify-end';
  ...
  return (
    <div className={`flex ${alignmentClass}`}>
      <div
        className={`mb-3 max-w-[80%] rounded-2xl px-4 py-3 text-sm transition-all duration-300 ease-in-out ${
          isStatus ? '' : 'shadow-md'
        } ${bubbleClasses}`}
      >
